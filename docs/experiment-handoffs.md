...
Integration base: `proposal/playable-map` while the proposal PR is open; updated `main` after it merges.
...
**Every session must create its own experiment branch and its own PR. Never implement on `main`, `proposal/playable-map`, or a branch shared with another session.** Use the unmerged proposal as the integration base, then switch to updated `main` once it merges. For concurrent sessions, use separate worktrees and coordinate changes to the shared experiment launcher and inspector. Each implementation stays on its own branch until its PR is reviewed and merged.
