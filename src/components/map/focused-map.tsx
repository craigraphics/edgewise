...
const INVITATIONS: Record<ExperimentId, { tint: string; title: string; blurb: string; action: string }> = {
  ...
  'generalization-overfitting': { tint: 'playable-invitation-learning', title: 'Does getting every past delivery right mean it will get the next one right?', blurb: 'Two rules, built from the same five deliveries. One matched every single one. Then try both on deliveries neither has seen.', action: 'Try the experiment' },
};
